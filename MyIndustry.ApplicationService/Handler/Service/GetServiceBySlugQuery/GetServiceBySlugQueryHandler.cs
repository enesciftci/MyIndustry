...
public class GetServiceBySlugQueryHandler : IRequestHandler<GetServiceBySlugQuery, GetServiceBySlugQueryResult>
{
    ...
    public async Task<GetServiceBySlugQueryResult> Handle(GetServiceBySlugQuery request, CancellationToken cancellationToken)
    {
        // Try to find by slug first, if not found try by ID (for backward compatibility)
        var service = await _serviceRepository
            .GetAllQuery()
            .FirstOrDefaultAsync(s => s.Slug == request.Slug || s.Id.ToString() == request.Slug, cancellationToken);
        ...
        return new GetServiceBySlugQueryResult()
        {
